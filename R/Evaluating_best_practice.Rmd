...
This figure tells us that the median (black) and the mean (grey) distance to nearest neighbour are very similar, meaning there is little skew, meaning again that the n reduction proccess has retained the systematic sampling design.

It also tells us that roughly 75% of the 
...
With distances above 25-30 m we start to se a systematic underestimation of volume. THe variation seems to be stable to about 30 m.
...
We can alternatively create groupes with similar ranges instead of similar n
```{r}
#summaryTable$group <- ifelse(
#  summaryTable$medianDist<20, "<20 m", ifelse(
#    summaryTable$medianDist<25, "20-25 m", ifelse(
#      summaryTable$medianDist<30, "25-30 m", ">30 m")))
#table(summaryTable$group)
```
...
```{r}
(cvPlot <- ggplot(data = cvTable, aes(x = order, y = cv))+
  geom_line(lty = 2)+
  geom_point(pch=21, fill="grey",
             size=3, stroke=1.5)+
  
  theme_bw(base_size = 16)+
  scale_x_continuous(breaks = cvTable$order,
                     labels = cvTable$label)+
