---
title: "Interpolation_Kinn_Marøy_202220222"
author: "MarteFandrem"
date: "22 2 2022"
output: pdf_document
---
...
```{r}
shp<-readOGR(dsn="C:/Users/martef/OneDrive - NTNU/Documents/Jobb-pc/PhD prosjekter/WP3 NINA GRAN/Data/Rydda datasett for kalkulator/Kinn", layer="maroy_avgrensing")

df <- read.csv("C:/Users/martef/OneDrive - NTNU/Documents/Jobb-pc/PhD prosjekter/WP3 NINA GRAN/Data/Rydda datasett for kalkulator/Kinn/torvdybder_maroy.csv", sep=";")
...
dfs <- st_transform(dfs, crs="+init=epsg:25833")
...
setwd('..')
tmap_save(data_Kinnm, "Output/Kinn/data_maroy.png")
data_Kinnm
...
setwd('..')
write.csv(volume, "Output/volume_Kinnm_20220221.csv")
```
...
#Subset to remove not peat
```{r}
dfs_peat <- subset(dfs, dfs$Dybde_m>0)

dfsp_peat <- as(dfs_peat, Class="Spatial")
```

#Interpolate all the various models again and get the volume
```{r, echo=FALSE}
neighbors = length(dfs_peat$Dybde_m)
power = c(seq(from = 1, to = 4, by = 1))
neigh = c((1), seq(from=2,to=30,by = 2), c(length=(neighbors)))
...
for (i in power) {
  for (j in neigh) {
    ...
    print(run)
    temp2 <- idw(Dybde_m ~ 1, dfsp_peat, grid_crop, nmax=j, idp=i)
    temp3 <- as.data.frame(temp2@data)
    temp4 <- sum(temp3$var1.pred)
    temp5 <- cbind(run, temp4)
    temp  <- rbind(temp, temp5)
...
```{r}
volume_peat <- temp
volume_peat <-dplyr::rename(volume_peat, volume=temp4)
volume_peat <- tidyr::separate(volume_peat, 
                        run, 
                        into = c("power", "nn"),
                        sep = "_",
                        remove=F)
volume_peat$power <- as.numeric(volume_peat$power)
volume_peat$nn <- as.numeric(volume_peat$nn)
volume_peat$volume <- as.numeric(volume_peat$volume)

setwd('..')
write.csv(volume_peat, "Output/volume_peat_Kinnm_20220221.csv")
```

#Values for printing (mean, min, max, SD)
```{r}
max <- max(volume_peat$volume)
min <- min(volume_peat$volume)
mean <- mean(volume_peat$volume)
sd <- sd(volume_peat$volume)


Description <- c("mean", "min", "max", "SD")
Results_volume_peat <- data.frame(Description, Results = c(mean, min, max, sd)) 
Results_volume_peat
```
...
for (i in power) {
  for (j in neigh) {
    ...
    print(run)
    temp2 <- krige.cv(Dybde_m ~ 1, dfsp_peat, nmax=j, set = list(idp=i))
    temp3 <- as.data.frame(temp2@data)
...
df_results <- temp
setwd('..')
write.csv(df_results, "Output/LOOCV_results_Kinnm_peat_20220221.csv")
```
...
setwd('..')
write.csv(df_agg, "Output/LOOCV_parameters_Kinnm_peat_20220221.csv")
```

```{r}
idw.best <- idw(Dybde_m ~ 1, dfsp, grid_crop, nmax=10, idp=2)
#
...
tm_shape(sf_shp)+
  ...
  tm_scale_bar(position = c("right", "bottom"), width = 0.3) +
     tm_layout(title="idp: 2, nn:10", inner.margins = c(0.25, 0.1, 0.1, 0.1), legend.show = TRUE, legend.position = c("left", "top"))

setwd('..')
tmap_save(map_bestfit, "Output/Kinn/bestfit.2_10_Kinnm.png")
map_bestfit
```
```{r}
idw.best2 <- idw(Dybde_m ~ 1, dfsp_peat, grid_crop, nmax=10, idp=2)
#

map_bestfit2 <- tmap_mode("plot") +
 tm_shape(idw.best2)+
  tm_raster(title= "Torvdybde (m)", col="var1.pred", palette="-viridis",breaks=seq(from = 0, to = 5, by = 0.5)) +
  tm_layout(legend.outside = TRUE)+
tm_shape(sf_shp)+
...
setwd('..')
tmap_save(map_bestfit2, "Output/Kinn/bestfit2.2_10_Kinnm.png")
map_bestfit2
```
...
setwd('..')
tmap_save(map_secondbest, "Output/Kinn/2_4_Kinnm.png")
map_secondbest
```
