...
```{r import shape files, message=F}
...
These to test cases are very different. Geilo is a set of several unique mire polygons. Usually, in development project, one would estimate the peat volume and C stock for each of these seperately, but we will try now to see if it can be done in one operation. Tydal is a more typical example of a clear peatland delimination. Both cases have dense peat depth measurements taken. 
